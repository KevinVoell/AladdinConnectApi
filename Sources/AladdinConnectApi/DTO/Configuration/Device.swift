...
public enum Ownership: String, Codable {
  case shared = "shared"
  case owned = "owned"
  case fullShare = "full-share"
}

public enum DeviceStatus: Int, Codable {
  case unknown = 0
...
public class Device : Codable, Identifiable {
  ...
  public init(fromJson json: JSON!){
    ...
    ownership = Ownership.init(rawValue: json["ownership"].stringValue)
    rssi = json["rssi"].intValue
    serial = json["serial"].stringValue
    ssid = json["ssid"].stringValue
    status = DeviceStatus.init(rawValue: json["status"].intValue)
  ...
  @objc public required init(coder aDecoder: NSCoder)
  {
    ...
    ownership = aDecoder.decodeObject(forKey: "ownership") as? Ownership
    ...
    status = aDecoder.decodeObject(forKey: "status") as? DeviceStatus
